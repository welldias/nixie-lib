...
static void draw_box(
    nixie_ascii_grid_t *g, int x, int y, int w, int h, const char *label, int rounded, int use_unicode) {
...
static int is_rounded_shape(nixie_node_shape_t shape) {
    return shape == NIXIE_SHAPE_ROUNDED || shape == NIXIE_SHAPE_STADIUM ||
           shape == NIXIE_SHAPE_CIRCLE || shape == NIXIE_SHAPE_DOUBLECIRCLE;
}
...
static uint32_t glyph_h(int use_unicode) { return use_unicode ? (uint32_t)0x2500 : (uint32_t)'-'; }
static uint32_t glyph_v(int use_unicode) { return use_unicode ? (uint32_t)0x2502 : (uint32_t)'|'; }

static int row_is_clear(const nixie_ascii_grid_t *g, int row, int col_start, int len) {
    if (row < 0 || row >= g->height) return 0;
    for (int c = col_start; c < col_start + len; c++) {
        if (c < 0 || c >= g->width) return 0;
        if (grid_get(g, c, row) != (uint32_t)' ') return 0;
    }
    return 1;
}
...
static ascii_edge_route_t compute_edge_route(
    int vertical_primary, int sx, int sy, int sw, int sh, int tx, int ty, int tw, int th) {
...
char *nixie_flowchart_render_ascii(
    ...
    int max_layer = 0;
    for (size_t i = 0; i < n; i++) {
        if (pf->nodes[i].layer > max_layer) max_layer = pf->nodes[i].layer;
    }
    ...
    for (int oi = 0; oi < non_empty; oi++) {
        ...
        for (int a = 0; a < count; a++) {
            ...
            if (extent > group_extent) group_extent = extent;
        }
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (right > canvas_w) canvas_w = right;
        if (bottom > canvas_h) canvas_h = bottom;
    }
    canvas_w += MARGIN;
    canvas_h += MARGIN;
    ...
    for (size_t i = 0; i < pf->edge_count; i++) {
        ...
        if (edge_channel[i] >= 0) {
            ...
            if (vertical_primary) {
                ...
                if (edge->has_arrow_end) grid_set(grid, nx, ny, '<');
                if (edge->has_arrow_start) grid_set(grid, ex, ey, '>');
            } else {
                ...
                if (edge->has_arrow_end) grid_set(grid, nx, ny, '^');
                if (edge->has_arrow_start) grid_set(grid, ex, ey, 'v');
            }
            continue;
        }

        ascii_edge_route_t route = compute_edge_route(
            vertical_primary, grid_x[s], grid_y[s], box_w[s], box_h[s], grid_x[t], grid_y[t], box_w[t], box_h[t]);
        ...
        if (edge->has_arrow_end) grid_set(grid, nx, ny, (uint32_t)route.arrow_end);
        if (edge->has_arrow_start) grid_set(grid, ex, ey, (uint32_t)route.arrow_start);
    }
