...
static void estimate_node_size(nixie_arena_t *arena, const char *label, nixie_node_shape_t shape, double *out_w, double *out_h) {
    nixie_multiline_metrics_t metrics =
        nixie_measure_multiline(arena, label, FONT_SIZE_NODE_LABEL, FONT_WEIGHT_NODE_LABEL);
...
static nixie_lg_direction_t to_lg_direction(nixie_direction_t d) {
    switch (d) {
        case NIXIE_DIR_TD: return NIXIE_LG_DOWN;
        case NIXIE_DIR_BT: return NIXIE_LG_UP;
        case NIXIE_DIR_LR: return NIXIE_LG_RIGHT;
        case NIXIE_DIR_RL: return NIXIE_LG_LEFT;
        default: return NIXIE_LG_DOWN;
    }
}
...
static nixie_point_t clip_to_diamond(nixie_point_t endpoint, nixie_point_t adjacent, nixie_rect_t box) {
    ...
    if (is_vertical) {
        ...
        if (dy > 0) {
            ok = (ray_x <= cx) ? intersect_vertical_ray(ray_x, left, top, &out)
                                : intersect_vertical_ray(ray_x, top, right, &out);
            if (!ok) out = top;
        } else {
            ok = (ray_x <= cx) ? intersect_vertical_ray(ray_x, bottom, left, &out)
                                : intersect_vertical_ray(ray_x, right, bottom, &out);
            if (!ok) out = bottom;
        }
    } else {
        double ray_y = endpoint.y;
        if (dx > 0) {
            ok = (ray_y <= cy) ? intersect_horizontal_ray(ray_y, top, left, &out)
                                : intersect_horizontal_ray(ray_y, left, bottom, &out);
            if (!ok) out = left;
        } else {
            ok = (ray_y <= cy) ? intersect_horizontal_ray(ray_y, top, right, &out)
                                : intersect_horizontal_ray(ray_y, right, bottom, &out);
            if (!ok) out = right;
        }
    }
...
static void clip_edge_to_shapes(
    nixie_point_t *points, size_t count, nixie_rect_t src_box, nixie_node_shape_t src_shape,
    nixie_rect_t dst_box, nixie_node_shape_t dst_shape) {
    if (count < 2) return;
...
nixie_positioned_flowchart_t *nixie_flowchart_layout(
    ...
    nixie_positioned_flowchart_t *pf =
        (nixie_positioned_flowchart_t *)nixie_arena_alloc_zeroed(arena, sizeof(nixie_positioned_flowchart_t));
    ...
    lg.edges = graph->edge_count > 0
                   ? (nixie_lg_edge_t *)nixie_arena_alloc_zeroed(arena, lg.edge_count * sizeof(nixie_lg_edge_t))
                   : NULL;
    ...
    pf->edges = pf->edge_count > 0
                    ? (nixie_pf_edge_t *)nixie_arena_alloc(arena, pf->edge_count * sizeof(nixie_pf_edge_t))
                    : NULL;
    ...
    for (size_t i = 1; i < pf->node_count; i++) {
        if (pf->nodes[i].x < min_x) min_x = pf->nodes[i].x;
        if (pf->nodes[i].y < min_y) min_y = pf->nodes[i].y;
