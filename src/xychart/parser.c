...
static void ensure_series_capacity(nixie_arena_t *arena, nixie_xy_chart_t *chart) {
    if (chart->series_count < chart->series_cap)
        return;
    ...
    if (chart->series_count > 0)
        memcpy(new_series, chart->series, chart->series_count * sizeof(nixie_xy_series_t));
...
static int try_title(nixie_arena_t *arena, const char *line, nixie_xy_chart_t *chart) {
    if (strncmp(line, "title", 5) != 0 || !(line[5] == ' ' || line[5] == '\t'))
        return 0;
    size_t i = 5;
    i += skip_ws(line + i);
    if (line[i] != '"')
    ...
    if (close == NULL || close == content_start)
        return 0; /* [^"]+ requires >= 1 char */
...
static int try_axis_categorical(nixie_arena_t *arena, const char *line, const char *kw, nixie_xy_axis_t *axis) {
    ...
    if (close_bracket == NULL || close_bracket == content_start)
        return 0;
...
nixie_xy_parse_result_t nixie_xy_parse(nixie_arena_t *arena, const char *text) {
    ...
    if (!(strncmp(header, "xychart", 7) == 0 && (header[7] == '\0' || !isalnum((unsigned char)header[7])))) {
        ...
        snprintf(result.error_message, sizeof(result.error_message), "Invalid mermaid header: \"%s\". Expected \"xychart-beta\".", header);
    ...
    for (size_t li = 1; li < sig.count; li++) {
        ...
        if (try_title(arena, line, chart))
            continue;
        if (try_axis_categorical(arena, line, "x-axis", &chart->x_axis))
            continue;
        if (try_axis_range(arena, line, "x-axis", &chart->x_axis))
            continue;
        if (try_axis_range(arena, line, "y-axis", &chart->y_axis))
            continue;
        if (try_y_axis_title_only(arena, line, &chart->y_axis))
            continue;
        if (try_series(arena, line, "bar", NIXIE_XY_BAR, chart))
            continue;
        if (try_series(arena, line, "line", NIXIE_XY_LINE, chart))
            continue;
