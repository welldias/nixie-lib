...
static void append_rounded_top_bar_path(nixie_strbuf_t *sb, double x, double y, double w, double h, double radius) {
    double rr = radius;
    if (w / 2.0 < rr) rr = w / 2.0;
    if (h / 2.0 < rr) rr = h / 2.0;
    if (rr <= 0.0) {
        nixie_strbuf_appendf(sb, "M%g,%g h%g v%g h%g Z", x, y, w, h, -w);
        return;
    }
    nixie_strbuf_appendf(sb,
        "M%g,%g Q%g,%g %g,%g L%g,%g Q%g,%g %g,%g L%g,%g Q%g,%g %g,%g L%g,%g Q%g,%g %g,%g Z",
        x, y + rr,
        x, y, x + rr, y,
        x + w - rr, y,
        x + w, y, x + w, y + rr,
...
static void append_rounded_right_bar_path(nixie_strbuf_t *sb, double x, double y, double w, double h, double radius) {
    ...
    nixie_strbuf_appendf(sb,
        "M%g,%g L%g,%g Q%g,%g %g,%g L%g,%g Q%g,%g %g,%g L%g,%g Q%g,%g %g,%g L%g,%g Q%g,%g %g,%g Z",
        x + rr, y,
        x + w - rr, y,
        x + w, y, x + w, y + rr,
        x + w, y + h - rr,
...
static const char *anchor_str(nixie_xy_text_anchor_t a) {
    switch (a) {
        case NIXIE_XY_ANCHOR_START: return "start";
        case NIXIE_XY_ANCHOR_END: return "end";
...
static void append_ticks(nixie_strbuf_t *sb, const nixie_xy_tick_t *ticks, size_t count, const char *fill) {
    for (size_t i = 0; i < count; i++) {
        nixie_strbuf_appendf(sb,
            "<text x=\"%g\" y=\"%g\" text-anchor=\"%s\" font-size=\"%g\" font-weight=\"%d\" dy=\"0.35em\" fill=\"%s\">",
            ticks[i].label_x, ticks[i].label_y, anchor_str(ticks[i].text_anchor), FONT_LABEL_SIZE, FONT_LABEL_WEIGHT, fill);
...
static void append_axis_title(nixie_strbuf_t *sb, const nixie_positioned_axis_t *axis, const char *fill) {
    ...
    nixie_strbuf_appendf(sb,
        "<text x=\"%g\" y=\"%g\" text-anchor=\"middle\"%s font-size=\"%g\" font-weight=\"%d\" dy=\"0.35em\" fill=\"%s\">",
        axis->title_x, axis->title_y, transform, FONT_AXIS_TITLE_SIZE, FONT_AXIS_TITLE_WEIGHT, fill);
...
char *nixie_xy_render_svg(
    ...
    for (size_t i = 0; i < pc->bar_count; i++) {
        if (pc->bars[i].color_index > max_idx) max_idx = pc->bars[i].color_index;
    }
    for (size_t i = 0; i < pc->line_count; i++) {
        if (pc->lines[i].color_index > max_idx) max_idx = pc->lines[i].color_index;
    }
    ...
    for (size_t i = 0; i < pc->line_count; i++) {
        if (pc->lines[i].point_count > max_line_points) max_line_points = pc->lines[i].point_count;
    }
    int sparse = max_line_points > 0 && max_line_points <= 12;

    for (size_t i = 0; i < pc->line_count; i++) {
        const nixie_positioned_line_t *line = &pc->lines[i];
        if (line->point_count == 0) continue;
    ...
    if (pc->has_title) {
        nixie_strbuf_appendf(&sb,
            "<text x=\"%g\" y=\"%g\" text-anchor=\"middle\" font-size=\"%g\" font-weight=\"%d\" dy=\"0.35em\" fill=\"%s\">",
            pc->title_x, pc->title_y, FONT_TITLE_SIZE, FONT_TITLE_WEIGHT, colors->text);
    ...
    for (size_t i = 0; i < pc->legend_count; i++) {
        ...
        nixie_strbuf_appendf(&sb,
            "<text x=\"%g\" y=\"%g\" text-anchor=\"start\" font-size=\"%g\" font-weight=\"%d\" dy=\"0.35em\" fill=\"%s\">",
            item->x + swatch_w + gap, item->y, FONT_LEGEND_SIZE, FONT_LEGEND_WEIGHT, colors->text_muted);
