...
static void grid_write(nixie_strbuf_t *sb, const nixie_ascii_grid_t *g) {
    ...
    for (int row = 0; row < g->height; row++) {
        for (int col = 0; col < g->width; col++) {
            if (grid_get(g, row, col) != (uint32_t)' ') {
...
static void draw_staircase_line_vertical(nixie_ascii_grid_t *g, const double *data, size_t data_count, int (*band_center)(int, void *), void *bc_ctx, int (*value_to_row)(double, void *), void *vr_ctx, int plot_top, int plot_h, int plot_left, int plot_total_w, const xy_glyphs_t *ch) {
    if (data_count == 0)
        return;
    ...
    for (size_t i = 0; i + 1 < data_count; i++) {
        ...
        for (int c = mid_col + 1; c <= p2.col; c++)
            grid_set(g, plot_top + (plot_h - 1 - p2.row), c, ch->h_line);

        if (i == 0) {
            int lead_start = p1.col - (p2.col - p1.col) / 4;
            if (lead_start < plot_left)
                lead_start = plot_left;
            for (int c = lead_start; c < p1.col; c++)
                grid_set(g, plot_top + (plot_h - 1 - p1.row), c, ch->h_line);
        }
        if (i + 2 == data_count) {
            int trail_end = p2.col + (p2.col - p1.col) / 4;
            if (trail_end > plot_left + plot_total_w - 1)
                trail_end = plot_left + plot_total_w - 1;
            for (int c = p2.col + 1; c <= trail_end; c++)
                grid_set(g, plot_top + (plot_h - 1 - p2.row), c, ch->h_line);
        }
    }
...
static void draw_staircase_line_horizontal(nixie_ascii_grid_t *g, const double *data, size_t data_count, int (*band_mid)(int, void *), void *bm_ctx, int (*value_to_col)(double, void *), void *vc_ctx, const xy_glyphs_t *ch) {
    if (data_count == 0)
        return;
    ...
    for (size_t i = 0; i + 1 < data_count; i++) {
        ...
        for (int c = min_col + 1; c < max_col; c++)
            grid_set(g, mid_row, c, ch->h_line);

        grid_set(g, mid_row, p2.col, going_right ? ch->corner_tr : ch->corner_tl);
        for (int r = mid_row + 1; r <= p2.row; r++)
            grid_set(g, r, p2.col, ch->v_line);
    }
...
static char *render_vertical(nixie_arena_t *arena, const nixie_xy_chart_t *chart, const xy_glyphs_t *ch) {
    ...
    if ((int)(data_count * 6) > plot_w)
        plot_w = (int)(data_count * 6);
    ...
    for (size_t i = 0; i < y_tick_count; i++) {
        ...
        int display_row = plot_top + (plot_h - 1 - row);
        for (int c = plot_left; c < plot_left + band_w * (int)data_count; c++) {
            if (grid_get(g, display_row, c) == (uint32_t)' ')
                grid_set(g, display_row, c, ch->grid);
        }
    }
        ...
        for (size_t si = 0; si < chart->series_count; si++) {
            ...
            for (size_t i = 0; i < s->data_count; i++) {
                ...
                for (int row = from_row; row <= to_row; row++) {
                    int display_row = plot_top + (plot_h - 1 - row);
                    for (int c = bx; c < bx + single_bar_w; c++)
                        grid_set(g, display_row, c, ch->bar);
                }
            }
            b_idx++;
        }
    ...
    for (size_t si = 0; si < chart->series_count; si++) {
        if (chart->series[si].type != NIXIE_XY_LINE)
            continue;
        const nixie_xy_series_t *s = &chart->series[si];
        draw_staircase_line_vertical(g, s->data, s->data_count, band_center_fn, &bc_ctx, value_to_row_fn, &vr_ctx, plot_top, plot_h, plot_left, band_w * (int)data_count, ch);
    }
...
static char *render_horizontal(nixie_arena_t *arena, const nixie_xy_chart_t *chart, const xy_glyphs_t *ch) {
    ...
    for (int r = plot_top; r < plot_top + plot_h; r++)
        grid_set(g, r, plot_left - 1, ch->v_line);
    grid_set(g, x_axis_row, plot_left - 1, ch->origin);
    ...
    for (size_t i = 0; i < value_tick_count; i++) {
        ...
        if (cx < plot_left || cx >= plot_left + plot_w)
            continue;
        for (int r = plot_top; r < plot_top + plot_h; r++) {
            if (grid_get(g, r, cx) == (uint32_t)' ')
                grid_set(g, r, cx, ch->grid);
        }
    }
