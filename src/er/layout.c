...
char *nixie_er_attribute_to_string(nixie_arena_t *arena, const nixie_er_attribute_t *attr) {
    ...
    if (attr->keys != 0) {
        ...
        if (attr->keys & NIXIE_ER_KEY_FK) {
            if (!first) nixie_strbuf_append_char(&sb, ',');
...
nixie_positioned_er_diagram_t *nixie_er_layout(nixie_arena_t *arena, const nixie_er_diagram_t *diagram) {
    nixie_positioned_er_diagram_t *pcd =
        (nixie_positioned_er_diagram_t *)nixie_arena_alloc_zeroed(arena, sizeof(nixie_positioned_er_diagram_t));
    ...
    for (size_t i = 0; i < diagram->entity_count; i++) {
        ...
        for (size_t k = 0; k < entity->attribute_count; k++) {
            ...
            if (w > max_attr_w) max_attr_w = w;
        }

        double width = ER_MIN_WIDTH;
        if (header_text_w + ER_BOX_PAD_X2 > width) width = header_text_w + ER_BOX_PAD_X2;
        if (max_attr_w + ER_BOX_PAD_X2 > width) width = max_attr_w + ER_BOX_PAD_X2;
    ...
    pcd->relationships = pcd->relationship_count > 0
                              ? (nixie_pe_relationship_t *)nixie_arena_alloc(arena, pcd->relationship_count * sizeof(nixie_pe_relationship_t))
                              : NULL;
