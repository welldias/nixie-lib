...
static void ensure_entity_capacity(nixie_arena_t *arena, nixie_er_diagram_t *d) {
    if (d->entity_count < d->entity_cap) return;
    ...
    if (d->entity_count > 0) memcpy(new_entities, d->entities, d->entity_count * sizeof(nixie_er_entity_t));
...
static void ensure_relationship_capacity(nixie_arena_t *arena, nixie_er_diagram_t *d) {
    ...
    nixie_er_relationship_t *new_rels =
        (nixie_er_relationship_t *)nixie_arena_alloc(arena, new_cap * sizeof(nixie_er_relationship_t));
    if (d->relationship_count > 0) memcpy(new_rels, d->relationships, d->relationship_count * sizeof(nixie_er_relationship_t));
...
static void ensure_attribute_capacity(nixie_arena_t *arena, nixie_er_entity_t *e) {
    if (e->attribute_count < e->attribute_cap) return;
    ...
    if (e->attribute_count > 0) memcpy(new_attrs, e->attributes, e->attribute_count * sizeof(nixie_er_attribute_t));
...
static int try_attribute(nixie_arena_t *arena, const char *line, nixie_er_attribute_t *out) {
    ...
    while (p < scan_end) {
        ...
        if (tok_len == 2) {
            ...
            if (c0 == 'P' && c1 == 'K') out->keys |= NIXIE_ER_KEY_PK;
            else if (c0 == 'F' && c1 == 'K') out->keys |= NIXIE_ER_KEY_FK;
            else if (c0 == 'U' && c1 == 'K') out->keys |= NIXIE_ER_KEY_UK;
        }
    }
...
static int parse_cardinality(const char *s, size_t len, nixie_er_cardinality_t *out) {
    ...
    if (lo == '|' && hi == '|') { *out = NIXIE_ER_ONE; return 1; }
    if (lo == 'o' && hi == '|') { *out = NIXIE_ER_ZERO_ONE; return 1; }
    if ((lo == '|' && hi == '}') || (lo == '{' && hi == '|')) { *out = NIXIE_ER_MANY; return 1; }
...
static int try_relationship(nixie_arena_t *arena, nixie_er_diagram_t *d, const char *line) {
    ...
    if (lend > lstart && (label_start[lstart] == '"' || label_start[lstart] == '\'')) lstart++;
    if (lend > lstart && (label_start[lend - 1] == '"' || label_start[lend - 1] == '\'')) lend--;

    nixie_er_cardinality_t c1, c2;
    if (!parse_cardinality(line + left_start, left_len, &c1)) return 0;
    if (!parse_cardinality(line + right_start, right_len, &c2)) return 0;
...
nixie_er_parse_result_t nixie_er_parse(nixie_arena_t *arena, const char *text) {
    ...
    if (!(strlen(sig.lines[0].content) == 9 && ieq_n(sig.lines[0].content, "erDiagram", 9))) {
        ...
        snprintf(result.error_message, sizeof(result.error_message),
                 "Invalid mermaid header: \"%s\". Expected \"erDiagram\".", sig.lines[0].content);
