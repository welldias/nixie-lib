...
static void render_attribute_row(
    nixie_strbuf_t *sb, const nixie_er_attribute_t *attr, double box_x, double y, double box_width,
    const nixie_resolved_colors_t *colors) {
    ...
    if (attr->keys != 0) {
        ...
        nixie_strbuf_appendf(sb, "<rect x=\"%g\" y=\"%g\" width=\"%g\" height=\"14\" rx=\"2\" ry=\"2\" fill=\"%s\" />",
            box_x + 6, y - 7, key_width, colors->key_badge);
        nixie_strbuf_appendf(sb,
            "<text x=\"%g\" y=\"%g\" text-anchor=\"middle\" dy=\"0.35em\" font-size=\"9\" font-weight=\"600\" fill=\"%s\">",
            box_x + 6 + key_width / 2.0, y, colors->text_sec);
    ...
    nixie_strbuf_appendf(sb,
        "<text x=\"%g\" y=\"%g\" class=\"mono\" dy=\"0.35em\" font-size=\"11\" font-weight=\"400\"><tspan fill=\"%s\">",
        type_x, y, colors->text_muted);
    append_escaped_xml(sb, attr->type, strlen(attr->type));
    nixie_strbuf_append(sb, "</tspan></text>");

    double name_x = box_x + box_width - 8.0;
    nixie_strbuf_appendf(sb,
        "<text x=\"%g\" y=\"%g\" class=\"mono\" text-anchor=\"end\" dy=\"0.35em\" font-size=\"11\" font-weight=\"400\"><tspan fill=\"%s\">",
        name_x, y, colors->text_sec);
    append_escaped_xml(sb, attr->name, strlen(attr->name));
    nixie_strbuf_append(sb, "</tspan></text>");

    if (has_comment) nixie_strbuf_append(sb, "</g>");
}
...
static void render_entity_box(nixie_strbuf_t *sb, const nixie_pe_node_t *entity, const nixie_resolved_colors_t *colors) {
    ...
    if (entity->attribute_count == 0) {
        nixie_strbuf_appendf(sb,
            "<text x=\"%g\" y=\"%g\" text-anchor=\"middle\" dy=\"0.35em\" font-size=\"11\" fill=\"%s\" font-style=\"italic\">(no attributes)</text>\n",
...
static void render_relationship_line(nixie_strbuf_t *sb, const nixie_pe_relationship_t *rel, const nixie_resolved_colors_t *colors) {
    ...
    nixie_strbuf_appendf(sb, "\" fill=\"none\" stroke=\"%s\" stroke-width=\"1\"%s />",
        colors->line, rel->identifying ? "" : " stroke-dasharray=\"6 4\"");
}
 ...
 */
static void render_crows_foot(
    nixie_strbuf_t *sb, nixie_point_t point, nixie_point_t toward, nixie_er_cardinality_t card,
    const nixie_resolved_colors_t *colors) {
    double sw = 1.0 + 0.25;
    ...
    if (has_one_line) {
        ...
        nixie_strbuf_appendf(sb, "<line x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" stroke=\"%s\" stroke-width=\"%g\" />",
            tip_x + px * half_w, tip_y + py * half_w, tip_x - px * half_w, tip_y - py * half_w, colors->line, sw);
        double line2x = tip_x - ux * 4.0, line2y = tip_y - uy * 4.0;
        nixie_strbuf_appendf(sb, "<line x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" stroke=\"%s\" stroke-width=\"%g\" />",
            line2x + px * half_w, line2y + py * half_w, line2x - px * half_w, line2y - py * half_w, colors->line, sw);
    }

    if (has_crows_foot) {
        double fan_w = 7.0;
        nixie_strbuf_appendf(sb, "<line x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" stroke=\"%s\" stroke-width=\"%g\" />",
            tip_x + px * fan_w, tip_y + py * fan_w, back_x, back_y, colors->line, sw);
        nixie_strbuf_appendf(sb, "<line x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" stroke=\"%s\" stroke-width=\"%g\" />",
            tip_x, tip_y, back_x, back_y, colors->line, sw);
        nixie_strbuf_appendf(sb, "<line x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" stroke=\"%s\" stroke-width=\"%g\" />",
            tip_x - px * fan_w, tip_y - py * fan_w, back_x, back_y, colors->line, sw);
    }
    ...
    if (has_circle) {
        ...
        nixie_strbuf_appendf(sb, "<circle cx=\"%g\" cy=\"%g\" r=\"4\" fill=\"%s\" stroke=\"%s\" stroke-width=\"%g\" />",
            cx, cy, colors->bg, colors->line, sw);
    }
}
...
static nixie_point_t arc_midpoint(const nixie_point_t *points, size_t count) {
    ...
    double total = 0.0;
    for (size_t i = 1; i < count; i++) total += point_dist(points[i - 1], points[i]);
...
static void render_relationship_label(nixie_strbuf_t *sb, const nixie_pe_relationship_t *rel, const nixie_resolved_colors_t *colors) {
    if (rel->label == NULL || rel->label[0] == '\0' || rel->point_count < 2) return;
