 ...
 * backends. */
static int position_notes_ascii(nixie_arena_t *arena, const nixie_seq_diagram_t *diagram, const int *ll_x, int after_index, int *cur_y_inout, note_pos_t *note_positions, size_t *note_count_out) {
    int any = 0;
...
char *nixie_sequence_render_ascii(nixie_arena_t *arena, const nixie_seq_diagram_t *diagram, const nixie_sequence_ascii_options_t *opts) {
    ...
    for (size_t i = 0; i < actor_count; i++) {
        ...
        if (actor_box_h[i] > actor_box_H)
            actor_box_H = actor_box_h[i];
    }
    ...
    for (size_t mi = 0; mi < diagram->message_count; mi++) {
        ...
        for (int g = lo; g < hi; g++) {
            if (per_gap > adj_max_width[g])
                adj_max_width[g] = per_gap;
        }
    }
    ...
    for (size_t i = 1; i < actor_count; i++) {
        ...
        if (adj_max_width[i - 1] + 2 > gap)
            gap = adj_max_width[i - 1] + 2;
        if (gap < 10)
            gap = 10;
        ll_x[i] = ll_x[i - 1] + gap;
    }
    ...
    for (size_t b = 0; b < diagram->block_count; b++) {
        block_start_y[b] = -1;
        block_end_y[b]   = -1;
        div_y[b]         = diagram->blocks[b].divider_count > 0 ? (int *)nixie_arena_alloc(arena, diagram->blocks[b].divider_count * sizeof(int)) : NULL;
        for (size_t d = 0; d < diagram->blocks[b].divider_count; d++)
            div_y[b][d] = -1;
    }

    note_pos_t *note_positions = diagram->note_count > 0 ? (note_pos_t *)nixie_arena_alloc(arena, diagram->note_count * sizeof(note_pos_t)) : NULL;
    ...
    for (size_t i = 0; i < note_pos_count; i++) {
        if (note_positions[i].x + note_positions[i].width + 1 > total_w)
            total_w = note_positions[i].x + note_positions[i].width + 1;
    }
    ...
    for (size_t i = 0; i < actor_count; i++) {
        int x = ll_x[i];
        for (int y = actor_box_H; y <= footer_y; y++)
            grid_set(grid, x, y, V);
    }
    ...
    for (size_t i = 0; i < actor_count; i++) {
        ...
        for (int pass = 0; pass < 2; pass++) {
            ...
            for (int x = 1; x < w - 1; x++)
                grid_set(grid, left + x, bottom_y, H);
    ...
    for (size_t m = 0; m < message_count; m++) {
        ...
        if (is_self) {
            ...
            for (int x = from_x + 1; x < from_x + loop_w; x++)
                grid_set(grid, x, y0, line_char);
            ...
            for (int x = from_x + 1; x < from_x + loop_w; x++)
                grid_set(grid, x, y0 + 2, line_char);
            ...
            if (left_to_right) {
                for (int x = from_x + 1; x < to_x; x++)
                    grid_set(grid, x, arrow_y, line_char);
    ...
    for (size_t b = 0; b < diagram->block_count; b++) {
        ...
        for (int x = b_left + 1; x < b_right; x++)
            grid_set(grid, x, top_y, H);
    ...
    for (size_t i = 0; i < note_pos_count; i++) {
        ...
        for (int x = 1; x < np->width - 1; x++)
            grid_set(grid, np->x + x, np->y, H);
