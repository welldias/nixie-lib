...
static void ensure_actor_capacity(nixie_arena_t *arena, nixie_seq_diagram_t *d) {
    ...
    if (d->actor_count > 0)
        memcpy(new_actors, d->actors, d->actor_count * sizeof(nixie_seq_actor_t));
    d->actors    = new_actors;
    d->actor_cap = new_cap;
}

static void ensure_message_capacity(nixie_arena_t *arena, nixie_seq_diagram_t *d) {
    if (d->message_count < d->message_cap)
        return;
    ...
    if (d->message_count > 0)
        memcpy(new_msgs, d->messages, d->message_count * sizeof(nixie_seq_message_t));
    d->messages    = new_msgs;
    d->message_cap = new_cap;
}

static void ensure_block_capacity(nixie_arena_t *arena, nixie_seq_diagram_t *d) {
    if (d->block_count < d->block_cap)
        return;
    ...
    if (d->block_count > 0)
        memcpy(new_blocks, d->blocks, d->block_count * sizeof(nixie_seq_block_t));
    d->blocks    = new_blocks;
    d->block_cap = new_cap;
}
...
static void ensure_stack_entry_divider_capacity(nixie_arena_t *arena, block_stack_entry_t *e) {
    if (e->divider_count < e->divider_cap)
        return;
    ...
    if (e->divider_count > 0)
        memcpy(new_divs, e->dividers, e->divider_count * sizeof(nixie_seq_divider_t));
    e->dividers    = new_divs;
...
static void stack_push(nixie_arena_t *arena, block_stack_t *s, block_stack_entry_t e) {
    if (s->count == s->cap) {
        ...
        if (s->count > 0)
            memcpy(new_entries, s->entries, s->count * sizeof(block_stack_entry_t));
...
static int ensure_actor(nixie_arena_t *arena, nixie_seq_diagram_t *d, const char *id, size_t id_len, const char *label, size_t label_len, nixie_seq_actor_type_t type) {
...
static int try_note(nixie_arena_t *arena, nixie_seq_diagram_t *d, const char *line) {
    if (!ieq_n(line, "Note", 4) || !(line[4] == ' ' || line[4] == '\t'))
        return 0;
...
static int try_block_start(nixie_arena_t *arena, block_stack_t *stack, nixie_seq_diagram_t *d, const char *line) {
    ...
    for (size_t k = 0; k < sizeof(kws) / sizeof(kws[0]); k++) {
        ...
        if (strncmp(line, kws[k].kw, kwlen) != 0)
            continue;
        if (line[kwlen] != '\0' && line[kwlen] != ' ' && line[kwlen] != '\t')
            continue;
...
nixie_sequence_parse_result_t nixie_sequence_parse(nixie_arena_t *arena, const char *text) {
    ...
    if (!starts_with_kw(sig.lines[0].content, "sequenceDiagram")) {
        ...
        snprintf(result.error_message, sizeof(result.error_message), "Invalid mermaid header: \"%s\". Expected \"sequenceDiagram\".", sig.lines[0].content);
    ...
    for (size_t li = 1; li < sig.count; li++) {
        ...
        if (try_actor_decl(arena, diagram, line))
            continue;
        if (try_note(arena, diagram, line))
            continue;
        if (try_block_start(arena, &stack, diagram, line))
            continue;
        if (try_divider(arena, &stack, diagram, line))
            continue;
        ...
        if (try_message(arena, diagram, line))
            continue;
