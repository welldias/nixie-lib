...
static int position_notes_for(nixie_arena_t *arena, const nixie_seq_diagram_t *diagram, const double *actor_center_x, const double *actor_widths, int after_index, double *note_y_inout, nixie_ps_note_t *notes, size_t *note_out_count) {
    int any       = 0;
    double note_y = *note_y_inout;

    for (size_t ni = 0; ni < diagram->note_count; ni++) {
        const nixie_seq_note_t *note = &diagram->notes[ni];
        if (note->after_index != after_index)
            continue;
        any = 1;

        double note_w = dmax(SEQ_NOTE_WIDTH, nixie_measure_text_width(note->text, FONT_SIZE_EDGE_LABEL, FONT_WEIGHT_EDGE_LABEL) + SEQ_NOTE_PAD_X * 2.0);
...
nixie_positioned_sequence_diagram_t *nixie_sequence_layout(nixie_arena_t *arena, const nixie_seq_diagram_t *diagram) {
    nixie_positioned_sequence_diagram_t *out = (nixie_positioned_sequence_diagram_t *)nixie_arena_alloc_zeroed(arena, sizeof(nixie_positioned_sequence_diagram_t));
    ...
    nixie_ps_message_t *messages = message_count > 0 ? (nixie_ps_message_t *)nixie_arena_alloc(arena, message_count * sizeof(nixie_ps_message_t)) : NULL;
    ...
    nixie_ps_activation_t *activations = message_count > 0 ? (nixie_ps_activation_t *)nixie_arena_alloc(arena, message_count * sizeof(nixie_ps_activation_t)) : NULL;
    size_t activation_count            = 0;

    act_stack_t *stacks = (act_stack_t *)nixie_arena_alloc_zeroed(arena, actor_count * sizeof(act_stack_t));

    nixie_ps_note_t *notes = diagram->note_count > 0 ? (nixie_ps_note_t *)nixie_arena_alloc(arena, diagram->note_count * sizeof(nixie_ps_note_t)) : NULL;
    ...
    for (size_t msg_idx = 0; msg_idx < message_count; msg_idx++) {
        ...
        for (size_t b = 0; b < diagram->block_count; b++) {
            ...
            if (blk->start_index == (int)msg_idx)
                extra = dmax(extra, SEQ_BLOCK_HEADER_EXTRA);
            for (size_t dv = 0; dv < blk->divider_count; dv++) {
                if (blk->dividers[dv].index == (int)msg_idx)
                    extra = dmax(extra, SEQ_DIVIDER_EXTRA);
            }
        }
        if (extra > 0.0)
            message_y += extra;
        ...
        int any_note_here      = position_notes_for(arena, diagram, actor_center_x, actor_widths, (int)msg_idx, &note_y, notes, &note_out_count);
    ...
    nixie_ps_block_t *blocks = diagram->block_count > 0 ? (nixie_ps_block_t *)nixie_arena_alloc(arena, diagram->block_count * sizeof(nixie_ps_block_t)) : NULL;
    ...
    for (size_t bi = 0; bi < diagram->block_count; bi++) {
        ...
        for (int mi = block->start_index; mi <= block->end_index; mi++) {
            ...
            if (min_idx < 0 || lo < min_idx)
                min_idx = lo;
            if (max_idx < 0 || hi > max_idx)
                max_idx = hi;
        }
        if (min_idx < 0) {
            min_idx = 0;
        ...
        pb->dividers      = block->divider_count > 0 ? (nixie_ps_divider_t *)nixie_arena_alloc(arena, block->divider_count * sizeof(nixie_ps_divider_t)) : NULL;
        for (size_t di = 0; di < block->divider_count; di++) {
            ...
            if (d->label[0] != '\0' && d_valid && messages[d->index].label[0] != '\0') {
                ...
                double msg_label_left  = msg->is_self ? msg->x1 + 36.0 : (msg->x1 + msg->x2) / 2.0 - msg_label_w / 2.0;
