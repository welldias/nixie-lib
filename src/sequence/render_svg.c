...
static void render_actor(nixie_strbuf_t *sb, const nixie_ps_actor_t *actor, const nixie_resolved_colors_t *colors) {
    ...
    if (actor->type == NIXIE_SEQ_ACTOR) {
        ...
        snprintf(label_attrs, sizeof(label_attrs),
            "font-size=\"%g\" text-anchor=\"middle\" font-weight=\"%d\" fill=\"%s\"", FONT_SIZE_NODE_LABEL,
            FONT_WEIGHT_NODE_LABEL, colors->text);
        ...
        nixie_strbuf_appendf(sb,
            "<rect x=\"%g\" y=\"%g\" width=\"%g\" height=\"%g\" rx=\"4\" ry=\"4\" fill=\"%s\" stroke=\"%s\" stroke-width=\"%g\" />\n",
...
static void render_lifeline(nixie_strbuf_t *sb, const nixie_ps_lifeline_t *ll, const nixie_resolved_colors_t *colors) {
    ...
    nixie_strbuf_appendf(sb, "\" x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" stroke=\"%s\" stroke-width=\"0.75\" stroke-dasharray=\"6 4\" />",
        ll->x, ll->top_y, ll->x, ll->bottom_y, colors->line);
}
...
static void render_activation(nixie_strbuf_t *sb, const nixie_ps_activation_t *act, const nixie_resolved_colors_t *colors) {
    ...
    nixie_strbuf_appendf(sb, "\" x=\"%g\" y=\"%g\" width=\"%g\" height=\"%g\" fill=\"%s\" stroke=\"%s\" stroke-width=\"%g\" />",
        act->x, act->top_y, act->width, act->bottom_y - act->top_y, colors->node_fill, colors->node_stroke, STROKE_WIDTH_INNER);
}
...
static void render_message(nixie_strbuf_t *sb, const nixie_ps_message_t *msg, const nixie_resolved_colors_t *colors) {
    ...
    nixie_strbuf_appendf(sb, "\" data-line-style=\"%s\" data-arrow-head=\"%s\" data-self=\"%s\">\n",
        msg->line_style == NIXIE_SEQ_DASHED ? "dashed" : "solid", msg->arrow_head == NIXIE_SEQ_FILLED ? "filled" : "open",
    ...
    if (msg->is_self) {
        ...
        nixie_strbuf_appendf(sb,
            "<polyline points=\"%g,%g %g,%g %g,%g %g,%g\" fill=\"none\" stroke=\"%s\" stroke-width=\"%g\"%s marker-end=\"url(#%s)\" />\n",
            msg->x1, msg->y, msg->x1 + loop_w, msg->y, msg->x1 + loop_w, msg->y + loop_h, msg->x2, msg->y + loop_h,
            colors->line, STROKE_WIDTH_CONNECTOR, dash_array, marker_id);

        snprintf(attrs, sizeof(attrs), "font-size=\"%g\" text-anchor=\"start\" font-weight=\"%d\" fill=\"%s\"",
            FONT_SIZE_EDGE_LABEL, FONT_WEIGHT_EDGE_LABEL, colors->text_muted);
        ...
        snprintf(attrs, sizeof(attrs), "font-size=\"%g\" text-anchor=\"middle\" font-weight=\"%d\" fill=\"%s\"",
            FONT_SIZE_EDGE_LABEL, FONT_WEIGHT_EDGE_LABEL, colors->text_muted);
...
static void render_block(nixie_strbuf_t *sb, const nixie_ps_block_t *block, const nixie_resolved_colors_t *colors) {
    ...
    double tab_w =
        nixie_measure_text_width(first_line_sb.data != NULL ? first_line_sb.data : "", FONT_SIZE_EDGE_LABEL, FONT_WEIGHT_GROUP_HEADER) +
        16.0;
    ...
    char label_attrs[160];
    snprintf(label_attrs, sizeof(label_attrs), "font-size=\"%g\" font-weight=\"%d\" fill=\"%s\"", FONT_SIZE_EDGE_LABEL,
        FONT_WEIGHT_GROUP_HEADER, colors->text_sec);
    ...
    for (size_t i = 0; i < block->divider_count; i++) {
        ...
        if (div->label != NULL && div->label[0] != '\0') {
            ...
            char div_attrs[160];
            snprintf(div_attrs, sizeof(div_attrs), "font-size=\"%g\" text-anchor=\"start\" font-weight=\"%d\" fill=\"%s\"",
                FONT_SIZE_EDGE_LABEL, FONT_WEIGHT_EDGE_LABEL, colors->text_muted);
...
static void render_note(nixie_strbuf_t *sb, const nixie_ps_note_t *note, const nixie_resolved_colors_t *colors) {
    ...
    nixie_strbuf_appendf(sb,
        "<polygon points=\"%g,%g %g,%g %g,%g\" fill=\"%s\" stroke=\"%s\" stroke-width=\"%g\" />\n",
