...
static int bary_cmp(const void *a, const void *b) {
    ...
    if (ea->bary > eb->bary)
        return 1;
    if (ea->orig_order < eb->orig_order)
        return -1;
    if (ea->orig_order > eb->orig_order)
        return 1;
    return 0;
...
nixie_lg_points_t nixie_lg_route_edge(nixie_arena_t *arena, const nixie_lg_graph_t *g, const nixie_lg_edge_t *e) {
    ...
    if (e->from < 0 || e->to < 0 || (size_t)e->from >= g->node_count || (size_t)e->to >= g->node_count) {
        return result;
    }
    ...
    if (layer_diff != 1) {
        ...
        if (!horizontal_primary_check) {
            ...
            pts[0].x      = src->x + src->width;
            pts[0].y      = ey;
            pts[1].x      = side_x;
            pts[1].y      = ey;
            pts[2].x      = side_x;
            pts[2].y      = ny;
            pts[3].x      = dst->x + dst->width;
            pts[3].y      = ny;
        } else {
            ...
            pts[0].x      = ex;
            pts[0].y      = src->y + src->height;
            pts[1].x      = ex;
            pts[1].y      = side_y;
            pts[2].x      = nx;
            pts[2].y      = side_y;
            pts[3].x      = nx;
            pts[3].y      = dst->y + dst->height;
        }
