...
char *nixie_class_member_to_string(nixie_arena_t *arena, const nixie_class_member_t *m) {
    ...
    if (m->visibility != NIXIE_VIS_NONE) {
        char v = '~';
        if (m->visibility == NIXIE_VIS_PUBLIC) v = '+';
        else if (m->visibility == NIXIE_VIS_PRIVATE) v = '-';
        else if (m->visibility == NIXIE_VIS_PROTECTED) v = '#';
    ...
    if (m->is_method) {
        ...
        nixie_strbuf_append_char(&sb, '(');
        if (m->params != NULL) nixie_strbuf_append(&sb, m->params);
        nixie_strbuf_append_char(&sb, ')');
    } else {
        nixie_strbuf_append(&sb, m->name);
    }
...
nixie_positioned_class_diagram_t *nixie_class_layout(nixie_arena_t *arena, const nixie_class_diagram_t *diagram) {
    nixie_positioned_class_diagram_t *pcd =
        (nixie_positioned_class_diagram_t *)nixie_arena_alloc_zeroed(arena, sizeof(nixie_positioned_class_diagram_t));
    ...
    for (size_t i = 0; i < diagram->class_count; i++) {
        ...
        double ah = cls->attribute_count > 0 ? (double)cls->attribute_count * CLS_MEMBER_ROW_HEIGHT + CLS_SECTION_PAD_Y
                                              : CLS_EMPTY_SECTION_HEIGHT;
        double mh = cls->method_count > 0 ? (double)cls->method_count * CLS_MEMBER_ROW_HEIGHT + CLS_SECTION_PAD_Y
                                           : CLS_EMPTY_SECTION_HEIGHT;
        header_h[i] = hh;
        attr_h[i] = ah;
        method_h[i] = mh;
        ...
        if (header_text_w + CLS_BOX_PAD_X2 > width) width = header_text_w + CLS_BOX_PAD_X2;
        if (max_attr_w + CLS_BOX_PAD_X2 > width) width = max_attr_w + CLS_BOX_PAD_X2;
        if (max_method_w + CLS_BOX_PAD_X2 > width) width = max_method_w + CLS_BOX_PAD_X2;
    ...
    pcd->relationships = pcd->relationship_count > 0
                              ? (nixie_pc_relationship_t *)nixie_arena_alloc(arena, pcd->relationship_count * sizeof(nixie_pc_relationship_t))
                              : NULL;
    ...
    for (size_t i = 1; i < pcd->node_count; i++) {
        if (pcd->nodes[i].x < min_x) min_x = pcd->nodes[i].x;
        if (pcd->nodes[i].y < min_y) min_y = pcd->nodes[i].y;
