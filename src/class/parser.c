...
static void ensure_class_capacity(nixie_arena_t *arena, nixie_class_diagram_t *d) {
    ...
    if (d->class_count > 0)
        memcpy(new_classes, d->classes, d->class_count * sizeof(nixie_class_node_t));
...
static void ensure_relationship_capacity(nixie_arena_t *arena, nixie_class_diagram_t *d) {
    ...
    nixie_class_relationship_t *new_rels = (nixie_class_relationship_t *)nixie_arena_alloc(arena, new_cap * sizeof(nixie_class_relationship_t));
    if (d->relationship_count > 0)
        memcpy(new_rels, d->relationships, d->relationship_count * sizeof(nixie_class_relationship_t));
...
static void ensure_attribute_capacity(nixie_arena_t *arena, nixie_class_node_t *c) {
    if (c->attribute_count < c->attribute_cap)
        return;
    ...
    if (c->attribute_count > 0)
        memcpy(new_attrs, c->attributes, c->attribute_count * sizeof(nixie_class_member_t));
    c->attributes    = new_attrs;
    c->attribute_cap = new_cap;
}

static void ensure_method_capacity(nixie_arena_t *arena, nixie_class_node_t *c) {
    if (c->method_count < c->method_cap)
        return;
    ...
    if (c->method_count > 0)
        memcpy(new_methods, c->methods, c->method_count * sizeof(nixie_class_member_t));
...
static int parse_member(nixie_arena_t *arena, const char *text, nixie_class_member_t *out) {
    ...
    if (len > 0 && (text[0] == '+' || text[0] == '-' || text[0] == '#' || text[0] == '~')) {
        switch (text[0]) {
        case '+':
            out->visibility = NIXIE_VIS_PUBLIC;
            break;
        case '-':
            out->visibility = NIXIE_VIS_PRIVATE;
            break;
        case '#':
            out->visibility = NIXIE_VIS_PROTECTED;
            break;
        default:
            out->visibility = NIXIE_VIS_PACKAGE;
            break;
        }
        pos = 1;
    ...
    size_t sp = 0;
    while (sp < rest_len && rest[sp] != ' ' && rest[sp] != '\t')
        sp++;
    ...
    if (sp < rest_len) {
        ...
        while (name_start < rest_len && (rest[name_start] == ' ' || rest[name_start] == '\t'))
            name_start++;
...
static int scan_class_header(const char *line, size_t *id_start, size_t *id_len, size_t *generic_start, size_t *generic_len, size_t *pos_out) {
...
static int try_relationship(nixie_arena_t *arena, nixie_class_diagram_t *d, const char *line) {
    ...
    if (*p != '\0')
        return 0; /* trailing garbage: not a valid relationship line */
...
nixie_class_parse_result_t nixie_class_parse(nixie_arena_t *arena, const char *text) {
    ...
    if (!(strlen(sig.lines[0].content) == 12 && ieq_n(sig.lines[0].content, "classDiagram", 12))) {
        ...
        snprintf(result.error_message, sizeof(result.error_message), "Invalid mermaid header: \"%s\". Expected \"classDiagram\".", sig.lines[0].content);
