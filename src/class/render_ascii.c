...
static void push_line(nixie_arena_t *arena, const char ***lines, size_t *count, size_t *cap, const char *line) {
    if (*count == *cap) {
        ...
        if (*count > 0) memcpy(new_lines, *lines, *count * sizeof(char *));
        *lines = new_lines;
        *cap = new_cap;
    }
    (*lines)[(*count)++] = line;
...
static void measure_box_content(const class_box_content_t *content, size_t *out_max_len) {
    ...
    for (size_t i = 0; i < content->line_count; i++) {
        if (content->lines[i] == DIVIDER_MARKER) continue;
...
static void draw_class_box(nixie_ascii_grid_t *g, int x, int y, int w, int h, const class_box_content_t *content, int use_unicode) {
    ...
    for (size_t i = 0; i < content->line_count; i++) {
        ...
        if (content->lines[i] == DIVIDER_MARKER) {
            ...
            for (int cx = x + 1; cx < x + w - 1; cx++) grid_set(g, cx, row, h_glyph);
            continue;
        }
...
static void draw_vline(nixie_ascii_grid_t *g, int col, int y1, int y2, int use_unicode) {
    ...
    for (int y = lo; y <= hi; y++) grid_set(g, col, y, glyph);
}
...
static void draw_hline(nixie_ascii_grid_t *g, int row, int x1, int x2, int use_unicode) {
    ...
    for (int x = lo; x <= hi; x++) grid_set(g, x, row, glyph);
}
...
static uint32_t marker_glyph(nixie_relationship_type_t type, int use_unicode) {
    switch (type) {
        ...
        case NIXIE_REL_REALIZATION: return use_unicode ? (uint32_t)0x25B2 /* ▲ */ : (uint32_t)'^';
        case NIXIE_REL_COMPOSITION: return use_unicode ? (uint32_t)0x25C6 /* ◆ */ : (uint32_t)'*';
        case NIXIE_REL_AGGREGATION: return use_unicode ? (uint32_t)0x25C7 /* ◇ */ : (uint32_t)'o';
        case NIXIE_REL_ASSOCIATION:
        case NIXIE_REL_DEPENDENCY:
        default: return use_unicode ? (uint32_t)0x25BA /* ► */ : (uint32_t)'>';
    }
}
...
char *nixie_class_render_ascii(
    ...
    int max_layer = 0;
    for (size_t i = 0; i < n; i++) {
        if (pcd->nodes[i].layer > max_layer) max_layer = pcd->nodes[i].layer;
    }
    int layer_count = max_layer + 1;

    int *layer_sizes = (int *)nixie_arena_alloc_zeroed(arena, (size_t)layer_count * sizeof(int));
    for (size_t i = 0; i < n; i++) layer_sizes[pcd->nodes[i].layer]++;
    ...
    typedef struct { int layer; double mean_primary; } layer_order_entry_t;
    int non_empty = 0;
    for (int l = 0; l < layer_count; l++) {
    ...
    for (int l = 0; l < layer_count; l++) {
        ...
        double sum = 0.0;
        for (int a = 0; a < layer_sizes[l]; a++) sum += pcd->nodes[layer_members[l][a]].y;
        order[oc].layer = l;
    ...
    for (int oi = 0; oi < non_empty; oi++) {
        ...
        for (int a = 0; a < count; a++) {
            ...
            if (box_h[idx] > group_extent) group_extent = box_h[idx];
        }
    ...
    for (size_t i = 0; i < pcd->relationship_count; i++) {
        ...
        if (rel->label != NULL && rel->label[0] != '\0') {
            ...
            if (grid_get(grid, label_col - 1, label_row) != (uint32_t)' ') label_col++;
