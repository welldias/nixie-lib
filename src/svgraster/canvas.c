...
static void fill_one_polygon(nixie_canvas_t *canvas, nixie_arena_t *arena, nixie_point_list_t points, nixie_rgb_t color, double opacity) {
...
static void stroke_run(nixie_canvas_t *canvas, nixie_arena_t *arena, const nixie_point_t *pts, int n, int closed, double half_width, nixie_rgb_t color, double opacity) {
    if (n < 1)
        return;
    ...
    for (int i = 0; i < n_edges; i++) {
        ...
        for (int k = 0; k < 4; k++)
            nixie_point_list_push(arena, &quad, corners[k]);
