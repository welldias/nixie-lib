...
static int utf8_decode(const char *s, int *out_cp) {
    ...
    if ((c0 & 0xF8) == 0xF0 && s[1] != '\0' && s[2] != '\0' && s[3] != '\0') {
        *out_cp = ((c0 & 0x07) << 18) | (((unsigned char)s[1] & 0x3F) << 12) | (((unsigned char)s[2] & 0x3F) << 6) |
...
void nixie_draw_text(nixie_canvas_t *canvas, const nixie_font_t *font, const nixie_svg_shape_t *shape) {
            ...
            if (bitmap != NULL) {
                for (int by = 0; by < gh; by++) {
                    for (int bx = 0; bx < gw; bx++) {
                        ...
                        if (lx < 0 || ly < 0 || lx >= buf_w || ly >= buf_h) continue;
    ...
    if (!shape->has_rotation) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (rp.x < dminx) dminx = rp.x;
            if (rp.x > dmaxx) dmaxx = rp.x;
            if (rp.y < dminy) dminy = rp.y;
            if (rp.y > dmaxy) dmaxy = rp.y;
        }
        ...
        if (dst_x1 > canvas->w) dst_x1 = canvas->w;
        if (dst_y1 > canvas->h) dst_y1 = canvas->h;
