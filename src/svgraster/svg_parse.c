...
static int next_tag(const char *p, tag_info_t *out) {
    ...
    while (q < gt && !is_ws(*q) && *q != '/' && *q != '>') q++;
    out->name = name_start;
    out->name_len = (size_t)(q - name_start);
    while (q < gt && is_ws(*q)) q++;
    out->attrs = q;
...
static const char *find_substr_bounded(const char *hay, size_t hay_len, const char *needle) {
    ...
    for (size_t i = 0; i + needle_len <= hay_len; i++) {
        if (strncmp(hay + i, needle, needle_len) == 0) return hay + i;
    }
    return NULL;
}
...
static int parse_color_attr(const char *attrs, size_t len, const char *key, nixie_rgb_t *out) {
    ...
    const char *v = find_attr(attrs, len, key, &vlen);
    if (v == NULL) return 0;
    if (vlen == 4 && strncmp(v, "none", 4) == 0) return 0;
    char buf[16];
    size_t n = vlen < sizeof(buf) - 1 ? vlen : sizeof(buf) - 1;
    memcpy(buf, v, n);
    buf[n] = '\0';
    nixie_rgb_t rgb;
    if (nixie_parse_hex(buf, &rgb) != 0) return 0;
    *out = rgb;
    return 1;
}
...
static nixie_point_list_t parse_points_attr(nixie_arena_t *arena, const char *s, size_t len) {
    ...
    while (p < end_bound) {
        while (p < end_bound && (is_ws(*p) || *p == ',')) p++;
        if (p >= end_bound) break;
        char *e;
        double x = strtod(p, &e);
        if (e == p) break;
        p = e;
...
static int parse_numbers_bounded(const char *p, const char *end_bound, double *out, int max_count) {
    ...
    while (n < max_count && p < end_bound) {
        ...
        p = e;
        while (p < end_bound && (*p == ',' || is_ws(*p))) p++;
        if (p < end_bound && *p == ')') break;
    }
    return n;
}

static void parse_transform_attr(const char *val, size_t val_len, nixie_affine_t *affine, int *has_rotate,
                                  double *rotate_deg, double *rotate_cx, double *rotate_cy) {
...
static void parse_marker_refs(const char *attrs, size_t len, nixie_svg_marker_ref_t *start, nixie_svg_marker_ref_t *end) {
    ...
    if (v != NULL) extract_marker_id(v, vlen, start);
    v = find_attr(attrs, len, "marker-end", &vlen);
    if (v != NULL) extract_marker_id(v, vlen, end);
}
...
static nixie_path_op_t *parse_path_d(nixie_arena_t *arena, const char *d, size_t len, int *out_count) {
    ...
    while (p < end_bound) {
        ...
        if (count == cap) {
            ...
            if (count > 0) memcpy(new_ops, ops, sizeof(nixie_path_op_t) * (size_t)count);
            ops = new_ops;
            cap = new_cap;
        }
        ops[count++] = op;
...
static void transform_path_ops_inplace(nixie_path_op_t *ops, int count, nixie_affine_t group, nixie_affine_t own) {
    for (int i = 0; i < count; i++) {
        ...
        if (op->kind == NIXIE_PATHOP_CLOSE) continue;
...
static void parse_svg_root(const char *attrs, size_t len, nixie_svg_document_t *doc, double scale) {
    ...
    if (doc->has_background) doc->background = bg;
}
...
static void push_marker(nixie_arena_t *arena, nixie_svg_marker_def_t **markers, int *count, int *cap,
                         nixie_svg_marker_def_t m) {
    if (*count == *cap) {
        int new_cap = (*cap == 0) ? 4 : (*cap) * 2;
        nixie_svg_marker_def_t *nm =
            (nixie_svg_marker_def_t *)nixie_arena_alloc(arena, sizeof(nixie_svg_marker_def_t) * (size_t)new_cap);
        if (*count > 0) memcpy(nm, *markers, sizeof(nixie_svg_marker_def_t) * (size_t)(*count));
        *markers = nm;
        *cap = new_cap;
    }
    (*markers)[(*count)++] = m;
...
static void push_shape(nixie_arena_t *arena, nixie_svg_shape_t **shapes, int *count, int *cap, nixie_svg_shape_t s) {
    if (*count == *cap) {
        ...
        if (*count > 0) memcpy(ns, *shapes, sizeof(nixie_svg_shape_t) * (size_t)(*count));
        *shapes = ns;
        *cap = new_cap;
    }
    (*shapes)[(*count)++] = s;
...
static const char *parse_text_block(nixie_arena_t *arena, tag_info_t text_tag, nixie_affine_t group,
                                     nixie_svg_shape_t **shapes, int *count, int *cap) {
    ...
    int has_tspan = next_tag(text_tag.after, &first_child) && first_child.tag_start < content_end &&
                    !first_child.is_end && tag_name_is(&first_child, "tspan");
    ...
    if (!has_tspan) {
        ...
        while (scan < content_end && next_tag(scan, &ct) && ct.tag_start < content_end) {
            if (!ct.is_end && tag_name_is(&ct, "tspan")) {
                ...
                if (parse_color_attr(ct.attrs, ct.attrs_len, "fill", &tfill)) thas_fill = 1;
                ...
                char *piece = ct.self_closing ? nixie_arena_strdup(arena, "")
                                               : unescape_xml_arena(arena, ct.after, (size_t)(tspan_content_end - ct.after));
...
static void parse_body(nixie_arena_t *arena, const char *svg_text, nixie_svg_document_t *doc, double scale) {
    ...
    while (next_tag(p, &t)) {
        if (t.is_end) {
            if (tag_name_is(&t, "g") && group_top > 0) group_top--;
            p = t.after;
            continue;
        }
        ...
        if (tag_name_is(&t, "rect")) {
            ...
            if (ptsv != NULL) {
                nixie_point_list_t pts = parse_points_attr(arena, ptsv, ptslen);
                for (int i = 0; i < pts.count; i++) pts.points[i] = affine_apply(group, pts.points[i]);
                s.points = pts.points;
                s.point_count = pts.count;
            }
            s.paint = parse_paint(t.attrs, t.attrs_len, base_scale_factor);
            if (is_polyline) parse_marker_refs(t.attrs, t.attrs_len, &s.marker_start, &s.marker_end);
            push_shape(arena, &shapes, &count, &cap, s);
...
nixie_svg_parse_result_t nixie_svg_parse(nixie_arena_t *arena, const char *svg_text, double scale) {
    ...
    if (doc->width <= 0.0 || doc->height <= 0.0) {
        ...
        strncpy(result.error_message, "could not determine SVG canvas dimensions (missing viewBox/width/height)",
