...
static const nixie_svg_marker_def_t *find_marker(const nixie_svg_document_t *doc, const char *id) {
    for (int i = 0; i < doc->marker_count; i++) {
        if (strcmp(doc->markers[i].id, id) == 0)
            return &doc->markers[i];
    }
    return NULL;
}
...
static void draw_marker(nixie_canvas_t *canvas, nixie_arena_t *arena, const nixie_svg_marker_def_t *m, nixie_point_t at, double angle_rad) {
    if (m->point_count < 3 || !m->has_fill)
        return;
...
static void draw_markers(nixie_canvas_t *canvas, nixie_arena_t *arena, const nixie_svg_document_t *doc, const nixie_svg_shape_t *s, const nixie_point_t *pts, int count) {
    if (count < 2)
        return;
...
static void draw_fill_stroke(nixie_canvas_t *canvas, nixie_arena_t *arena, nixie_point_list_t *poly, nixie_svg_paint_t paint) {
    if (poly->count < 3)
        return;
    ...
    if (paint.has_stroke) {
        nixie_canvas_stroke_polyline(canvas, arena, poly->points, poly->count, 1, paint.stroke_width, paint.stroke, paint.opacity, paint.dash_on, paint.dash_off);
    }
}
...
nixie_png_result_t nixie_svg_to_png_impl(const char *svg_text, const char *font_path, double scale) {
    ...
    if (need_font) {
        ...
        if (!font.ok) {
            nixie_arena_destroy(arena);
            return make_error(NIXIE_ERROR_INVALID_ARGUMENT, "failed to load font_path (not found or not a valid .ttf/.otf file)");
        }
    }
    ...
    for (int i = 0; i < doc->shape_count; i++) {
        ...
        case NIXIE_SVG_SHAPE_LINE: {
            ...
            if (s->paint.has_stroke) {
                nixie_canvas_stroke_polyline(canvas, arena, pts, 2, 0, s->paint.stroke_width, s->paint.stroke, s->paint.opacity, s->paint.dash_on, s->paint.dash_off);
            }
        ...
        case NIXIE_SVG_SHAPE_POLYLINE: {
            if (s->paint.has_stroke && s->point_count >= 2) {
                nixie_canvas_stroke_polyline(canvas, arena, s->points, s->point_count, 0, s->paint.stroke_width, s->paint.stroke, s->paint.opacity, s->paint.dash_on, s->paint.dash_off);
            }
        ...
        case NIXIE_SVG_SHAPE_PATH: {
            ...
            if (s->paint.has_stroke) {
                for (int li = 0; li < polys.count; li++) {
                    nixie_canvas_stroke_polyline(canvas, arena, polys.lines[li].points.points, polys.lines[li].points.count, polys.lines[li].closed, s->paint.stroke_width, s->paint.stroke, s->paint.opacity, s->paint.dash_on, s->paint.dash_off);
                }
            }
            break;
        }
